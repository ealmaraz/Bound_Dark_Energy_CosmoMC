    ...
    !erick13.01.15. Useful convertion factors for our model
    !  eV2toMpcm2    -> convertion factor: 1eV^2 = (eV2toMpcm2) Mpc^-2
    !  c8piG_eVm2    -> value of 8*pi*G in eV^-2
    !  Mpl2          -> squared reduced Planck's mass in eV^2
    !  gsmM          -> degrees of freedom (photons + 3 massless nu + electros + positrons) at nu_dec
    !  gsmgut        -> degrees of freedom in the MSSM at GUT scale
    real(dl), parameter :: eV2toMpcm2=(2._dl*const_pi*eV*Mpc/(c*h_P))**2
    real(dl), parameter :: c8piG_eVm2=16._dl*const_pi**2*G*eV**2/(h_P*c**5)
    real(dl), parameter :: Mpl2=h_P*c**5/(16._dl*const_pi**2*G*eV**2)
    real(dl), parameter :: gsmM=10.75
    real(dl), parameter :: gsmgut=228.75
