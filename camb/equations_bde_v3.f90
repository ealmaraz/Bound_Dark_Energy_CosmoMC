...
!!!!!
    ...
    module Quint
      ...
      integer, parameter :: NumPoints = 10000, NumPointsEx=NumPoints+2
     ...
     !erick30.11.19 - CosmoMC modification
     !we introduce these arrays for computing w0 & H0 in source/CosmologyParameterizations.f90
     real(dl) abde(NumPointsEx), Nbde(NumPointsEx), xbde(NumPointsEx), ybde(NumPointsEx), lbde(NumPointsEx)
     real(dl) ddxbde(NumPointsEx), ddybde(NumPointsEx), ddlbde(NumPointsEx)
    ...
    contains
      ...
      !erick30.11.19 - CosmoMC modification
      !Interpolation of xbde, ybde & lbde for the scale factor a.
      !Same approach as in Quint_ValsAta
      subroutine cosmomc_bde_xylAta(a,x,y,l)
      implicit none
      !Do interpolation at a
      !erick12.02.15. Given a, Narr is the previous lower value of the corresponding N.
      !               Within the arrays, this element has the index ix. We adapt the
      !               expressions for ix, a0, b03 and ho2o6. The interpolation formula
      !               of Quint_ValsAta is the same
      real(dl) a, x, y, l
      real(dl) a0,b0,ho2o6,a03,b03
      integer ix
      real(dl) Narr

      dN = log(1._dl/ac)/(NumPoints-1)
      ...
      a03=(a0**3-a0)
      b03=(b0**3-b0)
      !erick06.02.2017 This is the interpolation recipe
      x=a0*xbde(ix)+b0*xbde(ix+1)+(a03*ddxbde(ix)+b03*ddxbde(ix+1))*ho2o6
      y=a0*ybde(ix)+b0*ybde(ix+1)+(a03*ddybde(ix)+b03*ddybde(ix+1))*ho2o6
      l=a0*lbde(ix)+b0*lbde(ix+1)+(a03*ddlbde(ix)+b03*ddlbde(ix+1))*ho2o6
    end subroutine cosmomc_bde_xylAta
      ...
      subroutine bde_background
        ...
        !               present time.
        !erick30.11.19  CosmoMC modification. We now solve the background equations in
        !source/CosmologyParameterizations.f90. This is to plug our CAMB distribution into
        !CosmoMC without introducing too much extra code. Basically, this subroutine now does
        !nothing. Note also that we have ommitted the 'use_physical' flag from the arguments of
        !the subroutine (We only support use_physical = T at the moment)
        !use IniFile
        !use AMLutils
        !use constants
    ...
    subroutine init_background
      ...
      !erick30.11.19 CosmoMC modification. Since we only support 'use_physical = T' at the moment, we
      !remove 'uphys' from the arguments of 'bde_background'
      call bde_background
    ...
    contains
      ...
      recursive subroutine GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
        ...
        !erick30.11.19  CosmoMC modification. Since in the CosmoMC runs we evolve all of the modes
        !               and tauac is not a global parameter any longer, we comment these lines
    ...
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine initial(EV,y, tau)
        ...
        !erick30.11.19 CosmoMC modification. For some unexplained reason, the code breaks down
        !(segmentation fault) if we assume om = (grhob+grhoc)/sqrt(3*(grhog+grhonu+grhoDGc))
        !However, om is only used to compute omtau and subsequently to introduce a minor
        !correction term when setting the initial conditions. The effect of this correction
        !on aini and clxg (where the other perturbations are expressed from) is less than 0.01%
