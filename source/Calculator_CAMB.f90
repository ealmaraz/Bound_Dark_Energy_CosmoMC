    ...
    use CosmologyTypes
    use CosmoTheory
    !erick30.11.19 We keep w_lam & wa_ppf because these variables appear in camb/halofit_ppf
    !However, we don't use them
    ...
    !erick16.10.16. We connect the BDE parameters in CAMBParams (in CAMB) into CMBParams (in CosmoMC)
    P%alphaBDE = CMB%alphaBDE
    P%wDGc = CMB%wDGc
    P%Lambdac = CMB%Lambdac
    P%ac = CMB%ac
    P%cpot = CMB%cpot

    !erick03.03.17. Same connection between CAMBParams and CMBParams for the density of radiation today
    P%rhor0 = CMB%rhor0

    !erick. cross-check
    !write(*,*) 'Calculator_CAMB: Lc, ac, cpot, alpha = ', CMB%Lambdac, CMB%ac, CMB%cpot, CMB%alphaBDE
    ...
    !erick30.11.30. Values of alpha & wDGc. Anyway, these values are definitively
    !determined in CosmologyParametrizations.f90/SetForH using the same expressions
    P%alphaBDE = 2._mcp*(1._mcp+2._mcp/(Ncolors-Nflavors))
    P%wDGc = wDGc/3._mcp
