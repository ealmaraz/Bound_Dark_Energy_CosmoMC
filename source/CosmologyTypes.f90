    ...
    !erick21.01.18 For each extra z, max_derived_parameters is increased by 8:
    !              rsDV, Hubble, DA, FA in derived_theory.paramnames
    !              sigma8, fsigma8      in derived_LSS.paramnames
    !              xbde, ybde           in params_CMB_BDE.paramnames
    !              Since we have 37 redshift outputs and max_derived_parameters = 30 initially, we modify to
    !                                      30+37*(8) =326
    !integer, parameter :: max_derived_parameters = 30
    integer, parameter :: max_derived_parameters = 326
    ...
    Type TCosmoTheoryParams
        ...
        !erick21.01.18 We introduce selected extra z's
        real(mcp) :: z_outputs(37) = [0.001_mcp,0.106_mcp,0.150_mcp,0.200_mcp,0.320_mcp,0.400_mcp,0.500_mcp,&
             0.570_mcp,0.700_mcp,0.800_mcp,0.900_mcp,1.000_mcp,1.100_mcp,1.200_mcp,1.300_mcp,1.400_mcp,1.500_mcp,&
             1.600_mcp,1.700_mcp,1.800_mcp,1.900_mcp,2.000_mcp,2.100_mcp,2.200_mcp,2.300_mcp,2.400_mcp,2.500_mcp,&
             2.600_mcp,2.700_mcp,2.800_mcp,2.900_mcp,3.000_mcp,3.100_mcp,3.200_mcp,3.300_mcp,3.400_mcp,3.500_mcp]
    ...
    Type, extends(TTheoryParams) :: CMBParams
        ...
        real(mcp) h, H0, tau
        real(mcp) w, wa
        !erick30.11.19 BDE basic quantities in the CosmoMC framework
        !Only Lambdac & cpot can be sampled
        real(mcp) wDGc, cpot, alphaBDE, Lambdac
        !erick18.01.17 BDE derived quantities (100*theta_MC is derived!)
        real(mcp) ac, OmegaDGc, wx0, Nex, theta
        !erick03.03.17 Radiation energy density today (in eV**4)
        real(mcp) rhor0

        !erick21.01.18 Dynamical variables used to solve the background evolution in BDE
        !              The value of these variables at selected z's is exported to the chains
        !              See paramnames/params_CMB_BDE.paramnames
        real(mcp) , dimension(37) :: xdyn
        real(mcp) , dimension(37) :: ydyn
